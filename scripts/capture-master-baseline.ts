 ...
 * change) on a representative scene and pin the full members + fixtures
 * output. The movable-outlets byte-equality gate then asserts a scene with
 * zero device nodes/overrides still computes members STRICTLY byte-equal and
...
/** Representative scene: rect shell + interior partition, door + window,
 * kitchen/garage/bath/bed/hallway zones, slab, a placed toilet. Exercises
 * receptacle pairs on both interior faces, GFCI zones, door + hallway
 * switches, panel/meter, wiring, plumbing, hvac. */
...
console.log(
  ...
  target,
  Object.entries(out)
    .map(([k, v]) => `${k}: ${(v as { members: unknown[] }).members.length} members, ${(v as { fixtures: unknown[] }).fixtures.length} fixtures`)
    .join(' | '),
)
